...
function chatTimestamp(seconds) {
  ...
  if (stamp === today - dayMs) return "Ayer"
...
function dayLabel(seconds) {
  ...
  var stamp = startOfDay(date)
  if (stamp === today) return "Hoy"
  if (stamp === today - 86400000) return "Ayer"
...
function getFilePathFromDrop(drop) {
  if (!drop) return ""
  var raw = ""
  if (drop.hasUrls && drop.urls && drop.urls.length > 0) {
    raw = String(drop.urls[0])
  } else if (drop.hasText && drop.text) {
    raw = String(drop.text).trim()
  }
  if (!raw) return ""
  if (raw.indexOf("file://") === 0) {
    raw = raw.replace(/^file:\/\//, "")
  }
  raw = raw.split("\r")[0].split("\n")[0].trim()
...
function avatarInitials(name, kind) {
  ...
  var parts = n.split(/\s+/)
  if (parts.length >= 2) {
    var c1 = parts[0].charAt(0).toUpperCase()
    var c2 = parts[1].charAt(0).toUpperCase()
    if (/[A-Z0-9]/i.test(c1) && /[A-Z0-9]/i.test(c2)) return c1 + c2
  }
